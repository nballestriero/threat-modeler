...
export default function AssetInventory() {
  ...
  const handleSave = async () => {
    try {
      ...
      setSelectedAsset(null);
      fetchAssets(); // ✅ Sincronizza store globale
    } catch (err) {
  ...
  const handleEnhance = async (id) => {
    const targetId = id || assets[0]?.id;
    if (!targetId) return alert('Seleziona un asset o creane uno nuovo prima di usare l\'AI');
    try {
      const res = await axios.post(`http://localhost:3001/api/assets/${targetId}/enhance`);
      if (res.data.enhanced !== false) fetchAssets();
      else alert(res.data.message || 'Nessuna modifica applicata');
    } catch {
      alert('Errore comunicazione AI');
    }
  };
        ...
        <table className="w-full text-left">
          ...
          <tbody>
            ...
            {assets.length === 0 && !isLoading && (
              <tr><td colSpan="4" className="p-4 text-center text-gray-400">Nessun asset. Clicca "Nuovo Asset" per iniziare.</td></tr>
            )}
          </tbody>
        </table>
      </div>
